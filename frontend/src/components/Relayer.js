...
const Relayer = (props) => {
  ...
  const [destinationAmt, setDestinationAmt] = useState({ erc20: 0, eth: 0 });

  useEffect(() => {
    const updateBal = async () => {
      const { erc20, eth, dEth, dErc20 } = await updateBalance(
        provider,
        zkWallet,
        erc20Contract,
        props.destination
      ...
      setDestinationAmt((prevState) => {
        return { ...prevState, erc20: dErc20, eth: dEth };
      });
      setZkWalletAmt((prevState) => {
        return { ...prevState, erc20, eth };
      });
    };
    if (zkWallet) {
      updateBal();
    }
  }, [props.destination, props.raiser]);
  ...
  const generateRootAndDeployContract = async () => {
    ...
    var erc20Contract = await deployErc20(
      provider,
      zkWallet.address,
      erc20Address
    );
    ...
    const { erc20, eth } = await updateBalance(
      provider,
      zkWallet,
      erc20Contract
    );
      ...
      <div className="shadow sm:overflow-hidden sm:rounded-md">
        <div className="space-y-6 bg-white px-4 py-5 sm:p-6">
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="container">
              <label htmlFor="erc20Addres">Destination ETH Amount: </label>
              <p>{destinationAmt.eth / 1e18} ethers</p>
              <label htmlFor="erc20Addres">Destination ERC20 Amount: </label>
            ...
            <MemberComponents raiser={props.raiser} userList={props.userList} />
          </div>
        </div>
      </div>
    </div>
